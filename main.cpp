...
const unsigned int windowSize_x = 1800;
const unsigned int windowSize_y = 700;
const unsigned int groundOffset = windowSize_y - 160.f;
int gameSpeed = 8;
...
class SoundManager
{
    public:
        ...
        :dieBuffer(), jumpBuffer(), pointBuffer(), dieSound(), jumpSound(), pointSound()
        {
            ...
            jumpBuffer.loadFromFile("rsrc/Sounds/oppa1.wav");
            pointBuffer.loadFromFile("rsrc/Sounds/point1.wav");
...
class Ground
{
    ...
    {
        if(groundTexture.loadFromFile("rsrc/Images/GroundImage1.png"))
        {
 ...
 void update(sf::Time& deltaTime)
{
    ...
    if (playerDead == true)
    {
        for (auto& bobstacle : obstacles) 
        {
            bobstacle.obstacleSprite.move(0.f, 0.f);
        }
    }
}
        ...
        void drawTo(sf::RenderWindow& window)
        {
            for(auto& dobstacles : obstacles)
            {
                window.draw(dobstacles.obstacleSprite);
            }
        }
...
class Dino
{
    public:
        ...
        void update(sf::Time& deltaTime, std::vector<Obstacle>& obstacles)
        {
            ...
            for(auto& sobstacles: obstacles)
            {
                if(dinoBounds.intersects(sobstacles.obstacleBounds))
                {
                    playerDead = true;
                }
            }
            ...
            if(!playerDead)
            {
                ...
                {
                    dinoMotion.y += 1.f; dino.setTextureRect(frames[1]);// sakrash 
                }
        ...
        void reset()
        {
            dinoMotion.y = 0; dino.setPosition(sf::Vector2f(dino.getPosition().x, windowSize_y - 500.f));
            dino.setTextureRect(frames[0]);
...
int main()
{
    sf::RenderWindow window(sf::VideoMode(windowSize_x, windowSize_y), "Qilich Game Production");
    ...
    while(window.isOpen())
    {
      ...
      //  window.loadFromFile("rsrc/Images/paralla.png");
